...
//#ifdef DEBUG_DISAS
    //add capture scsi commands
    //uint64_t  ts = get_timestamp_micro();//add
    if (unlikely(qemu_loglevel_mask(CPU_LOG_TB_SCSI_CMD))) {
	FILE *logfile = qemu_log_lock();
        qemu_log("SCSI-CMD----------------\n");
        //qemu_log("%lld, %d, %d, %d, %ld ,%d, %d\n",ts, req->cmd.buf[0], req->cmd.len, req->cmd.xfer, req->cmd.lba, req->cmd.mode, req->dev->id); 
        qemu_log("%d %d %ld ", req->cmd.buf[0], req->cmd.len, req->cmd.xfer);
        qemu_log("%ld ,%d ", req->cmd.lba, req->cmd.mode);
        qemu_log("general\n");
        qemu_log_unlock(logfile);
    }
//#endif
	//qemu_log("test\n");
	//printf("%ld, %d, %d, %ld, %ld ,%d, id%d, xxxx\n",ts, req->cmd.buf[0], req->cmd.len, req->cmd.xfer, req->cmd.lba, req->cmd.mode, req->dev->id);
...
//#ifdef DEBUG_DISAS
    //add scsi commands dma
    //uint64_t  ts = get_timestamp_micro();//add
    if (unlikely(qemu_loglevel_mask(CPU_LOG_TB_SCSI_CMD))) {
	FILE *logfile = qemu_log_lock();
        qemu_log("SCSI-CMD----------------\n");
        //qemu_log("%ld, %d, %d, %d, %lld, %d, %d\n",ts, req->cmd.buf[0], req->cmd.len, req->cmd.xfer, req->cmd.lba, req->cmd.mode, req->dev->id);
        qemu_log("%d %d %ld ", req->cmd.buf[0], req->cmd.len, req->cmd.xfer);
        qemu_log("%ld ,%d ", req->cmd.lba, req->cmd.mode);
        qemu_log("dma\n");
        qemu_log_unlock(logfile);
