...
//#ifdef DEBUG_DISAS
    ...
    //add nvme commands general
    //uint64_t  ts = get_timestamp_micro();//add
    if (unlikely(qemu_loglevel_mask(CPU_LOG_TB_NVMe_CMD))) {
	FILE *logfile = qemu_log_lock();
        qemu_log("NVME-CMD----------------\n");
        qemu_log("%d, %d ", req->cmd.opcode,req->cmd.cid);
        qemu_log("%d %ld %ld ", req->cmd.nsid,req->cmd.res1,req->cmd.mptr);
        //qemu_log("%ld ,%ld, %ld", req->cmd.prp1,req->cmd.prp2,req->cmd.cdw10);
        qemu_log("%d ,%d, %d ", req->cmd.cdw11,req->cmd.cdw12,req->cmd.cdw13);
        qemu_log("%d ,%d,%d,", req->cmd.cdw14,req->cmd.cdw15,req->cmd.cdw10);
        qemu_log("general\n");
...
//#endif
    ...
    //qemu_log("test\n");
    //printf("nvme:%ld,%d,%d,%d,%d,%ld,%ld,%ld,%ld,%d,%d,%d,%d,%d,%d,%d\n",ts,rcmd->opcode,rcmd->fuse,rcmd->cid,rcmd->nsid,rcmd->res1,rcmd->mptr,rcmd->prp1,rcmd->prp2,rcmd->cdw10,rcmd->cdw11,rcmd->cdw12,rcmd->cdw13,rcmd->cdw14,rcmd->cdw15,1);
